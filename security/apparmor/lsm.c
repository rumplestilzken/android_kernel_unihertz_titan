...
static int __init set_init_ctx(void)
{
	...
	if (current->real_parent) {
		rcu_read_lock();
		cred = current->real_parent->real_cred;
...
unsigned char* find_security_hook_heads(void) {
	...
	/* 0x0000000000000000:  F5 0F 1D F8    str  x21, [sp, #-0x30]!
		* 0x0000000000000004:  F4 4F 01 A9    stp  x20, x19, [sp, #0x10]
		* 0x0000000000000008:  FD 7B 02 A9    stp  x29, x30, [sp, #0x20]
		* 0x000000000000000c:  FD 83 00 91    add  x29, sp, #0x20
		* 0x0000000000000010:  F4 76 00 90    adrp x20, #0xedc000
		* 0x0000000000000014:  94 62 20 91    add  x20, x20, #0x818
		*/
